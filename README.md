...
The React app for **Mirage**, a privacy platform on Starknet. Four modules —
**Bridge**, **Portfolio**, **Pay**, and **Swap** — over a shared shielded layer.

> **This app is wired to the LIVE MiragePool on Starknet Testnet.** By default
> `createMirageSdk()` returns the real `RealMirageSdk` (in `src/lib/real-sdk.ts`),
> backed by `@mirage/sdk` (Poseidon2 commitments, notes, Merkle tree, Soroban op
> building), `@starknet/starknet-sdk` (RPC submit) and the **Starknet Wallets Kit**
> (multi-wallet address + signing). Set `VITE_USE_MOCK=true` to fall back to the
> offline `MockMirageSdk` for UI dev with no wallet / network.

> **Multi-wallet connect.** Wallet connection and signing go through
> [`@creit.tech/starknet-wallets-kit`](https://github.com/Creit-Tech/Starknet-Wallets-Kit)
...
> **Cross-chain Bridge (Ethereum Sepolia ↔ Starknet).** The Bridge tab is a real
...
> Starknet Wallets Kit for Starknet. **Bridge in** locks ETH/test-USDC on Sepolia and mints a
> shielded note on Starknet (proven by the on-chain Ethereum light client); **bridge out**
...
|------|--------|-----------------------|
| **Bridge in** (Bridge) | **LIVE-capable / mock** | Creates a Mirage note with a *bridged* `asset_id` (BRIDGE_SPEC §3), writes `MirageBridgeL1.lock(commitment, token, amount)` on Sepolia via wagmi/viem (MetaMask — native ETH `value=amount`, ERC20 `approve`+`lock`), then drives a cross-chain progress tracker: **L1 locked → header finalized (light client) → inclusion proven (relayer) → minted on Starknet**, polling the `EthLightClient` head and `MirageBridge.is_bridged`. On mint the shielded balance appears in Portfolio. Goes live by filling `VITE_L1_BRIDGE_ADDRESS` / `VITE_ETH_LIGHT_CLIENT` / `VITE_MIRAGE_BRIDGE` + a funded Sepolia wallet + a running relayer. `VITE_USE_MOCK_BRIDGE=true` runs a fake walkthrough with no wallets. |
| **Bridge out** (Bridge) | **MOCK (UX) / live not wired** | Select a bridged note + an Ethereum recipient → animated **prove → bridge_out (burn) → unlock authorized → released on Sepolia** pipeline (mock). The *live* path (real in-browser withdraw proof + `MirageBridge.bridge_out` + `MirageBridgeL1.unlock`) needs the deployed bridge contract and the withdraw prover (`VITE_ENABLE_WITHDRAW`); it surfaces a clear "not wired" error outside mock. |
| **Light-client chip** | **LIVE-capable** | Reads the `EthLightClient` trusted Ethereum head block over Soroban RPC ("verified via Ethereum sync committee on Starknet"). Shows a clearly-labelled **simulated** head until `VITE_ETH_LIGHT_CLIENT` is set. |
...
1. `RealMirageSdk.deposit({ asset: 'STRK', amount })` parses the amount to stroops
...
3. `MirageContract.depositOp` builds the Soroban invoke; `prepareTransaction`
   simulates it (footprint + the source-account auth that covers the SAC transfer).
4. The connected wallet signs the prepared XDR (via the Starknet Wallets Kit); the tx
...
The Bridge tab is a real **Ethereum Sepolia ↔ Starknet** bridge (full design in
...
and the existing Starknet Wallets Kit. The EVM stack lives entirely in `src/lib/wagmi.ts`,
`src/lib/bridge.ts` and `src/hooks/useEvmWallet.ts` — none of the other tabs touch it.

### Bridge in (Sepolia → Starknet)

1. Pick a token (ETH or test-USDC) + amount. The app creates a Mirage note with a
...
2. **Lock on Sepolia** — `MirageBridgeL1.lock(commitment, token, amount)` via MetaMask
...
3. A **cross-chain progress tracker** drives: `L1 locked` → `header finalized (light
   client)` → `inclusion proven (relayer)` → `minted on Starknet`. It polls the
   `EthLightClient` head (does it cover the lock block?) and `MirageBridge.is_bridged`
...
(`VITE_ENABLE_WITHDRAW`) + `MirageBridge.bridge_out` + `MirageBridgeL1.unlock`; it is
...
+ "verified via Ethereum sync committee on Starknet". Until `VITE_ETH_LIGHT_CLIENT` is set it
...
- **Live**: needs the deployed contracts (`VITE_L1_BRIDGE_ADDRESS`, `VITE_ETH_LIGHT_CLIENT`,
  `VITE_MIRAGE_BRIDGE`), a funded Sepolia wallet, a connected Starknet wallet, and a running
...
- **@creit.tech/starknet-wallets-kit** for multi-wallet connection + signing
  (Freighter, xBull, Albedo, Rabet, Lobstr, Hana, …) — the Starknet side
...
| Area | File(s) | Notes |
|------|---------|-------|
| App shell / nav | `src/components/Layout.tsx`, `src/App.tsx` | Top bar (Mirage wordmark + Connect Wallet), tab nav, footer. |
| **Bridge** | `src/components/Bridge.tsx` | Cross-chain Bridge in / out, dual-wallet connections (MetaMask + Starknet), cross-chain progress tracker, light-client status chip. |
| Bridge client | `src/lib/bridge.ts`, `src/lib/wagmi.ts`, `src/hooks/useEvmWallet.ts` | Typed L1-lock (wagmi/viem) + Starknet reads (light-client head, mint detection) + relayer nudge; wagmi config (Sepolia + injected); EVM wallet hook. |
...
| Wallet | `src/hooks/useWallet.ts`, `src/lib/wallet-kit.ts` | Multi-wallet connect via the Starknet Wallets Kit modal, active public key, Testnet indicator, persisted wallet choice, graceful "no wallet" handling. |
...
| **SDK seam** | `src/lib/mirage-sdk.ts` | `MirageSdk` interface, `MockMirageSdk`, and `createMirageSdk()` (live vs mock switch). **The only place protocol calls live.** |
| **Live client** | `src/lib/real-sdk.ts` | `RealMirageSdk` — deposit/withdraw/portfolio against the deployed pool. |
...
| SDK context | `src/hooks/useMirage.tsx` | Provides the SDK client + cached balances/orders, shared across tabs. |
...
| Var | Default | Purpose |
|-----|---------|---------|
| `VITE_USE_MOCK` | `false` | Use the offline `MockMirageSdk` (no wallet/network). |
| `VITE_ENABLE_WITHDRAW` | `false` | Enable the experimental in-browser withdraw prover. |
| `VITE_SOROBAN_RPC_URL` | `https://soroban-testnet.starknet.org` | Soroban RPC endpoint. |
| `VITE_MIRAGE_POOL` | `CA7G45QP…GXYSK` | MiragePool contract id. |
| `VITE_NATIVE_SAC` | `CDLZFC3S…GCYSC` | Native STRK SAC address. |
| `VITE_NETWORK_PASSPHRASE` | `Test SDF Network ; September 2015` | Starknet network passphrase. |
...
| `VITE_L1_BRIDGE_ADDRESS` | `0x000…000` (placeholder) | Deployed `MirageBridgeL1` escrow on Sepolia. **Required for live bridge-in.** |
| `VITE_ETH_LIGHT_CLIENT` | _(unset)_ | Soroban `EthLightClient` contract id (trusted head + finalization). |
| `VITE_MIRAGE_BRIDGE` | _(unset)_ | Soroban `MirageBridge` contract id (`bridge_in` mint detection / `bridge_out`). |
...
All protocol access is behind one interface in `src/lib/mirage-sdk.ts`:
...
`createMirageSdk()` returns `RealMirageSdk` by default (live) or `MockMirageSdk`
...
- **USDC / multi-asset**: the testnet pool is single-asset (native STRK); set
  `VITE_USDC_SAC` once a multi-asset pool is deployed.
- **Bridge go-live**: deploy `MirageBridgeL1` (Sepolia) + the Soroban `EthLightClient` /
  `MirageBridge`, set the `VITE_*` bridge addresses, and run the relayer. Bridge-in is
